...
public class ClientController {
    ...
    public @ResponseBody List listAllClients(Model model) {
        List<Client> clientList = clientService.showAllClients();
        List<Client> rezList = new LinkedList<>();
        for (Client client: clientList){
            client.setClientTotalOrders(null);
            rezList.add(client);
        }
        return rezList;
    }
    ...
    public @ResponseBody List listClients (Model model){

        List list = clientService.showClientsLastMonth();
